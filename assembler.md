...
Код программы состоит из команд конкретного ассемблера, команд процессора и меток.

Например, `.quad` и `.string` - это **команды ассемблера**, они выполняются при сборке программы и сохраняют произвольное значения в тело программы для каких-либо операций над ними при работе программы.

**Команды процессора** переводятся в свой машинные эквиваленты - опкоды или коды операций. Например, `mov` или `syscall` - это текстовое представление машинной команды процессора, которая в готовой программе выглядит, как значение длиной в байт, обозначающий эту команду в определённой архитектуре процессора. После команды могут идти ещё несколько байт, являющихся операндами - конкретным значением, адресом значения или адресом назначения в случае, если это команда копирования `mov`. Процессор читает эти команды из участка памяти запущенной программы в оперативной памяти и по очереди выполняет.
...
- `movl` - doubleword, 4 байта, 32 бита
- `movq` - quadword, 8 байт, 64 бита
...
Управление потоком команд осуществляется с помощью команд условного перехода. Они используют регистр machine status word, который содержит результат последней арифметической операции или команды `cmp`.
...
Для работы со значениями в оперативной памяти используются адреса ячеек, которые указываются в виде метки или регистра (содержащего адрес) в качестве операнда команды процессора.

- `%rdx` - значение в регистре
- `(%rdx)` - значению по адресу из регистра (разыменование указателя в Си)
- `16(%rdx)` - значение из регистра с смещением адреса на +16 байт/ячеек
- `-8(%rdx)` - значение из регистра с смещением адреса на -8 байт/ячеек
- `(%rdx, %rcx, 8)` - обращение к массиву (адрес массива, индекс и размер элемента)
- `8(,%rax,4)` - RAX \* 4 + 8
- `$var1` - метка (заменяется в готовой программе на адрес)
- `(var1)` - значение в метке
- `(var1 + 1)` - со смещением
...
Область памяти в конце адресного пространства процесса. Используется для хранения значений подпрограмм. Команда `push` копирует в стопку значение в старший адрес. Последующие копирования помещают значения ниже предыдущего. Команда `pop` забирает последнее значение, при этом оно удаляется, а RSP вырастает.
...
адреса ячеек памяти
  ...
  0x7fff3fedd780 │       значение 5       │
                 ├────────────────────────┤
  0x7fff3fedd778 │       значение 6       │ RSP, текущая вершина, pop заберёт его первым
                 ├────────────────────────┤
                 │                        │ push поместит новое значение сюда
                 └────────────────────────┘
```
...
Отдельная последовательность команд с меткой, к которой можно перейти, выполнить и вернутся обратно.

```assembly
call sub1 # адрес возврата сохраняется в стек, указатель команд переходит к метке
# команды

sub1:
# команды

ret # адрес возврата забирается из стека, указатель команд переходит к адресу возврата
```

При работе подпрограммы обычно используют стопку. Туда временно сохраняются значения регистров для их использования в других целях, любые произвольные значения и выделяется диапазон пространства под временные значения. В итоге мы получаем кадр стопки (stack frame). Для обращения к значениям в стопке используется смещение регистра RBP, в который сохраняется RSP в начале подпрограммы.
...
mysubprogram:
    ...
    # Сохраняем текущий RBP в стопку для последующего восстановления
    push %rbp

    # Сохраняем текущую вершину стека в RBP, чтобы обращаться
    # к другим значениям в стопке через смещение
    mov %rsp, %rbp

    # При необходимости выделяем в стопке пространство под переменные,
    # сместив RSP на нужный объём, например, 16 байт
    sub $16, %rsp

    # Если RBX, RDI или RSI будут задействованы в подпрограмме
    # для других целей, то их нужно сохранить
    push %rbx
    push %rdi
    push %rsi
    ...
    # Помещаем результат работы в RAX (ноль обычно означает успех)
    mov $0, %rax

    # Восстанавливаем RBX, RDI, RSI в обратном порядке
    pop %rsi
    pop %rdi
    pop %rbx

    # Если выделялось пространство под переменные,
    # то освобождаем его уменьшением RSP или переносом RBP в RSP
...
_start:
    ...
    # то сохраняем в стек для последующего восстановления
    push %rax
    push %rcx
    push %rdx

    # Сохраняем в стек значения для подпрограммы
    push $8
    push $somestring

    # Переходим в подпрограмму по метке и сохраняем адрес возврата в стопку
    call mysubprogram

    # Очищаем стек от переданных значений, например,
    # увеличив указатель RSP на использованный объём
    add $16, %rsp

    # Восстанавливаем RAX, RCX, RDX
    pop %rdx
    pop %rcx
    pop %rax
...
При вложенных переходах в подпрограммы кадры стопок выстраиваются друг после друга.
...
Собираем программу с отладочной информацией для связи машинных команд со строками из исходного кода

    as -g prog1.s -o prog1.o && ld prog1.o -o prog1

Запускаем отладчик

    gdb --args prog1 arg1 arg2

Включаем отображение исходного кода с текущей позицией в программе

    la src

Если в программе нет отладочных меток, то можно отобразить код ассемблера

    la asm

Включаем отображение регистров процессора

    la r

Назначаем точку остановки

    b _start

Начинаем выполнение программы до первой точки остановки

    r

Выполняем одну машинную команду

    si

Далее нажатием Enter повторяется последняя команда. В процессе выполнения можно наблюдать прогресс выполнения программы в исходном коде, а также значения регистров процессора.

Отобразить содержимое памяти по адресу или метке
...
[https://cutter.re](https://cutter.re) - отладчик с интерфейсом
...
- перемещение процессора в точку входа программы

При загрузке в память заменяется копия процесса, созданного командой fork. Также заново создаётся стек, куча (heap) и раздел изменяемых данных.

## Микрокод

Часть сложных команд процессора могут быть сделаны не физически на схеме, а в виде микропрограммы для устройства, управляющего процессором. В основном это относится к процессорам архитектуры CISC, а в RISC - наоборот сохраняется минимальный набор простых команд, выполненных на схеме.
