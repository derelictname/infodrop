...
```assembly
myvar1:             # метка
.byte 255           # сохранить число по адресу $myvar1
.byte 0b10101010    # доступно по смещению $myvar1 + 1

myvar2:
.word 0x310a
.word 0x320a        # $myvar2 + 2

myvar3:
.quad 0x32ac68fce170539f
.long 0x18e458d2    # размер 64-битного адреса
...
В x86 для системных вызовов используется программное прерывание `int 0x80`, но из-за разности привелегий процесса и обработчика процессор делал много лишний действий для переноса данных в стек обработчика. В x86-64 появилась команда `syscall`, которая быстро переходит на уровень привелегий обработчика.
...
- `$rbp` - значение регистра
- `(%rbp)` - значение по адресу из регистра (разыменование указателя
- `16(%rbp)` - значение по адресу из регистра со смещением (увеличением адреса)
- `-8(%rbp)` - значение по адресу из регистра со смещением (уменьшением адреса)
- `(%rsp, %rcx, 8)` - RSP + RCX * 8, где RCX играет роль индекса массива, а 8 - размер элементов
- `$var` - значение по метке
- `$var + 1` - значение по метке со смещением
- `$var(, %rcx, 8)`

Примеры

- `mov  (%rsp), %rax` - при запуске программы RSP указывает на вершину стека с числом переданных программе аргументов (сохраняем его в RAX)
...
Способ временного выделения памяти для программы с помощью команд `push` и `pop`. Память выделяется с наибольшего адреса (с конца) в сторону уменьшения. При выполнении `push` в стек сохраняется значение, при этом увеличивается регистр RSP на длину значения. RSP всегда содержит адрес последнего значения в стеке (вершина стека).
...
Выделение памяти путём смещения конца выделенной процессу памяти через системный вызов `brk`. Также `mmap`. Память выделяется после text, data. Растёт в сторону увеличения адреса.
...
_start:
        ...
        mov $12, %rax
        syscall               # выполняем brk(0) для получения начала кучи

        # brk возвращает в RAX либо адрес, либо -1

        mov %rax, %r8         # забираем начало кучи

        # раз уж память всё равно выделяется страницами по 4096 байт, то выделим полностью

        lea 4096(%rax), %rdi  # получаем конец кучи через смещение и записываем в аргумент для brk
        mov $12, %rax         # выполняем brk(100) для смещения конца программы
        syscall

        cmp $-1, %rax         # если brk не удался, то перейти в exit
        jz exit

        mov $53, %dl          # записываем значение для записи в кучу
        mov $0, %rbx          # записываем начальный индекс значения в куче

save:
        mov %dl, (%r8, %rbx)  # записали
        inc %rbx              # увеличили индекс
        cmp $4096, %rbx       # если индекс достиг конца кучи, то перейти в exit
        jz free
        jmp save              # повторить
