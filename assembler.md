...
mov $4, %eax     # sys_write
mov $1, %ebx     # stdout
mov $mystr, %ecx # начало данных
mov $8, %edx     # длина данных
int $0x80        # сигнал для Linux обработать системный вызов

mov $1, %eax     # sys_exit
mov $0, %ebx     # значение выхода
int $0x80        # сигнал для Linux обработать системный вызов
```
...
Для языка ассемблера нет стандарта, кроме документации от производителя процессора. Наиболее распространённой архитектурой процессора является Intel x86.
...
Подпрограмма - объединение нескольких команд процессора с меткой вначале для многоразового использования. Функции и процедуры из высокоуровневых языков программирования становятся подпрограммами. При использовании подпрограмм в памяти временно выделяется пространство для временного хранения значений. Запись и считывание туда выполняются в формате стека, который заполняется в сторону уменьшения адресов.
...
```
 ...
 ▲   │    сохранённый RAX    │
 │ а ├───────────────────────┤
 │ д │ переданное значение 3 │  32($rbp)
 │ р ├───────────────────────┤
 │ е │ переданное значение 2 │  24($rbp)
 │ с ├───────────────────────┤
 │ а │ переданное значение 1 │  16($rbp)
 │   ├───────────────────────┤
     │    адрес возврата     │
     ├───────────────────────┤
     │    сохранённый RBP    │
...
Также при оформлении подпрограмм есть ряд правил для поддержания корректной работы программы.

### 1. Перед вызовом

Если в RAX, RCX или RDX есть важные значения, то сохранить нужные в стек для последующего восстановления

    push %rax
    push %rcx
    push %rdx

Записать в стек любые значения, которые понадобяться внутри подпрограммы

    push $8
    push $mystr

Выполнить переход в подпрограмму, указав метку (это также сохранит адрес возврата в стек)
...
Записать RSP в RBP (адрес в RBP будет использоваться для запроса к другим значениям в стеке)

    mov %rsp, %rbp

При необходимости выделить в стеке пространство под переменные, например, сместив адрес RSP на их суммарный размер

    sub $8, %rsp # выделили 8 байт для переменных

Если RBX, RDI или RSI будут использоваться в подпрограмме, то сохранить нужные в стек для последующего восстановления
...
- `-lс` - добавление динамической зависимости с `/lib/x86_64-linux-gnu/libc.so`
