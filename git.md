...
Объекты хранятся в папке `.git/objects`. Содержимое текстового файла появилось в виде blob в файле `.git/objects/f0/2e02a059f190d3e68d42f8438fa3f094eda1b5`. Имя папки вместе с именем это чексумма SHA1 содержимого blob. Это образует **контекстно-адресуемое** хранилище, в котором расположение файлов связано с их содержимым.

Имена файлов и их расположения в папках хранится в виде объектов **tree**. Один tree содержит информацию о именах и чексуммах файлов в одной папке, а также других tree, которые хранят информацию о вложенных папках.
...
Комбинация из объектов blob и tree позволяет фиксировать изменения в основной папке. Так Git ведёт учёт истории изменений.
...
В commit можно добавлять изменения файлов частями
...
Помимо фиксации изменений в истории необходимо также уметь возвращаться к старым изменениям. Для этого используется `git checkout`.

С помощью `git checkout` можно выгрузить в текущую папку старое состояния файлов, указав branch, commit или tree (можно не полностью)

    git checkout 9dd5bt4

Загрузить файлы, но не перемещать HEAD

- `git reset eb21e` - загрузить только в index
- `git reset --hard eb21er3` - загрузить в index и в рабочую папку

`checkout` и `reset` не затрагивают в папке файлы, которые никогда не добавлялись в Git.

- `git clean` - удалить неизвестные для Git файлы в рабочей папке

Если нам не нравятся изменения, сделанные в каком-либо конкретном commit, то можно создать его обратную версию
