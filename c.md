...
#define NUMBER int // определяет текст для подстановки во всем коде

int main(void)
{
        NUMBER a = 10;

#ifdef NUMBER // условное использование кода
        a = a + 10;
#endif
}
```
...
Данный пример показывает типичную ситуацию с возникновением рекурсии, так как две функции бесконечно вызывают друг друга. Это приведёт к переполнению стека вызовов, так как процессор хранит в нём переменные, используемые в функции до их завершения, а функции не завершаются. После переполнение стека произойдёт попытка обращения с следующему участку памяти, который недоступен, что называется ошибкой [сегментации](https://ru.wikipedia.org/wiki/Сегментная_адресация_памяти).
...
**Неявное преобразование** - неявное изменение типа переменной при операциях над разными типами. Чтобы код программы выглядел более очевидно, рекомендуется делать явное преобразование.
...
**Структура** - составная переменная, хранящая в себе несколько переменных (похоже на класс данных в Python).
...
**Синоним типов** - назначение произвольного имени для типа данных. Используется для удобства написания программы и читаемости. Часто при определении используют анонимную инициализацию типа данных.
...
С помощь макросов производится предварительная обработка кода

    cpp myprogram.c > myprogram.i

Трансляция в ассемблер (с отключением отладочных команд)

    gcc -S -fno-asynchronous-unwind-tables myprogram.i -o myprogram.s

Преобразование кода в объектный файл-модуль, который содержит машинный код и данные

    as myprogram.s -o myprogram.o

Компоновка/линковка вместо меток/символов прописывает адреса

    ld myprogram.o -dynamic-linker /lib64/ld-linux-x86-64.so.2 /usr/lib/x86_64-linux-gnu/Scrt1.o -o myprogram -lc
